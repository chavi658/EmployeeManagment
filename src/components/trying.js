...
const handleRoleSelection = (event, roleId) => {
  ...
  if (isChecked) {
    // אם התפקיד כבר קיים, אל תוסיף אותו למערך תפקידים
    if (!selectedRoles.has(roleId)) {
      setSelectedRoles(prevRoles => new Set(prevRoles.add(roleId)));
    }
  } else {
    // אם התפקיד לא קיים, הסר אותו מהמערך תפקידים
    if (selectedRoles.has(roleId)) {
